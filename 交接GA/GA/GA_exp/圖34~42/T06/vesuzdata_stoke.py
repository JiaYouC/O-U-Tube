# -*- coding: utf-8 -*-
"""
Created on Wed Aug 23 02:58:22 2023
...
with open('stoke.csv' , 'r') as csv_file:
...
for num in range(0,len(T),1):
    ...
    if num ==0:
        function=32*np.sin(2*np.pi*ttt/T[num])+32/4*np.sin(4*np.pi*ttt/T[num]+np.pi*3/2)
        count=0
        cnt=0  
        while cnt!=-1:
              if(function[count]<0):
        ...
        tf=tf/100
        function=(32*np.sin(2*np.pi*tf/T[num])+32/4*np.sin(4*np.pi*tf/T[num]+np.pi*3/2))/40 
        array_all = np.hstack((tt,function,vfourier))
    if num!=0:
        array_all = np.hstack((array_all,vfourier))
a=array_all.reshape(len(T)+2,-1).T
np.savetxt("stoke_vesuz.csv",a, delimiter=',',fmt='%1.12f')