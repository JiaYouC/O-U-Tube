...
with open('forward.csv' , 'r') as csv_file:
...
for num in range(0,len(T),1):
    ...
    if num ==0:
        function=109*np.sin(2*np.pi*ttt/T[num])+109/4*np.sin(4*np.pi*ttt/T[num])
        count=0
        cnt=0  
        while cnt!=-1:
              if(function[count]<0):
        ...
        tf=tf/100
        function=(109*np.sin(2*np.pi*tf/T[num])+109/4*np.sin(4*np.pi*tf/T[num]))/120 
        array_all = np.hstack((tt,function,vfourier))
    if num!=0:
        array_all = np.hstack((array_all,vfourier))
a=array_all.reshape(len(T)+2,-1).T
np.savetxt("fordward_vesuz.csv",a, delimiter=',',fmt='%1.12f')
#%%